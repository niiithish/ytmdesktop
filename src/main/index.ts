...
const createOrShowSettingsWindow = (): void => {
  ...
  settingsWindow = new BrowserWindow({
    width: 560,
    height: 420,
    x: Math.round(mainWindowBounds.x + (mainWindowBounds.width / 2 - 280)),
    y: Math.round(mainWindowBounds.y + (mainWindowBounds.height / 2 - 210)),
    minimizable: false,
    maximizable: false,
    resizable: false,
    show: false,
  ...
  settingsWindow.on("ready-to-show", () => {
    // Scale down the content to fit the smaller window (70% of original)
    settingsWindow.webContents.setZoomFactor(0.7);
    settingsWindow.show();
    // Open the DevTools.
    if (process.env.NODE_ENV === "development") {
...
app.on("ready", async () => {
  ...
  ipcMain.on("settingsWindow:open", event => {
    // Allow both main window and ytmView to open settings
    if (event.sender !== mainWindow.webContents && (ytmView === null || event.sender !== ytmView.webContents)) return;
